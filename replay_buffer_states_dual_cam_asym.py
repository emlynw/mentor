...
class ReplayBuffer(IterableDataset):
    def __init__(self, replay_dir, frame_stack, max_size, num_workers, nstep, discount,
                 fetch_every, save_snapshot, sample_key1, sample_key2):
        self._replay_dir = replay_dir
        self._frame_stack = frame_stack
        self._size = 0
        ...
        self.sample_key1 = sample_key1
        self.sample_key2 = sample_key2
    ...
    def _sample(self):
        ...
        img1 = episode[self.sample_key1][idx-self._frame_stack:idx]
        img1 = img1.reshape(img1.shape[0]*img1.shape[1], *img1.shape[2:])
        img2 = episode[self.sample_key2][idx-self._frame_stack:idx]
        img2 = img2.reshape(img2.shape[0]*img2.shape[1], *img2.shape[2:])
        next_img1 = episode[self.sample_key1][idx+self._nstep-self._frame_stack:idx+self._nstep]
        next_img1 = next_img1.reshape(next_img1.shape[0]*next_img1.shape[1], *next_img1.shape[2:])
        next_img2 = episode[self.sample_key2][idx+self._nstep-self._frame_stack:idx+self._nstep]
        next_img2 = next_img2.reshape(next_img2.shape[0]*next_img2.shape[1], *next_img2.shape[2:])
        state = episode['state'][idx-1]
        next_state = episode['state'][idx+self._nstep-1]
        priv_state = episode['priv_state'][idx-1]
        next_priv_state = episode['priv_state'][idx+self._nstep-1]
        ...
        return (img1, img2, state, priv_state, action, reward, mask, next_img1, next_img2, next_state, next_priv_state)
...
def make_replay_loader(replay_dir, frame_stack, max_size, batch_size, num_workers,
                       save_snapshot, nstep, discount, sample_key1, sample_key2):
    ...
    iterable = ReplayBuffer(replay_dir,
                            ...
                            sample_key1=sample_key1,
                            sample_key2=sample_key2)
