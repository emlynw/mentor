...
class DrMAgent:
    ...
    def update(self, replay_iter, step):
        ...
        if self.dormant_perturb_interval > 0 and step % self.dormant_perturb_interval == 0:
            self.perturb(step)
            self.perturb_time += 1
        ...
        self.dormant_ratio, metrics = utils.cal_dormant_ratio(
            self.actor, obs.detach(), 0, percentage=self.dormant_threshold, metrics=metrics)
        
        # print(self.dormant_ratio)
