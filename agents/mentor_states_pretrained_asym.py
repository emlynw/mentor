...
class MENTORAgent:
    def __init__(self, obs_shape, action_shape, discount, nstep, device, lr, feature_dim,
        ...
        self.value_predictor = VNetwork(obs_shape, feature_dim, hidden_dim, state_dim, priv_state_dim).to(device)
        self.critic = Critic(obs_shape, action_shape, feature_dim, hidden_dim, state_dim, priv_state_dim).to(device)
        self.critic_target = Critic(obs_shape, action_shape, feature_dim, hidden_dim, state_dim, priv_state_dim).to(device)
    ...
    def update(self, replay_iter, step):
        ...
        obs, obs_sensor, priv_state, action, reward, mask, next_obs, next_obs_sensor, next_priv_state = utils.to_torch(batch, self.device)
